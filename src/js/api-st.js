...
export default class StocksAPI {
	...
	static addHandlers() {
		chrome.webRequest.onBeforeSendHeaders.addListener(
			...
			{ urls: ["https://www.alphavantage.co/create_post/"] },
			["blocking", "requestHeaders", "extraHeaders"]
		);

		chrome.webRequest.onHeadersReceived.addListener(
			function (details) {
				const cookie = details.responseHeaders.find(
					obj => obj.name === "Set-Cookie"
				).value;
			...
			{ urls: ["https://www.alphavantage.co/support/*"] },
	...
	static async SYMBOL_SEARCH(keywords) {
		const res = await fetch(
			`https://www.alphavantage.co/query?function=SYMBOL_SEARCH&keywords=${keywords}&apikey=${localStorage.getItem(
				"keyST"
			)}`
	...
	// 	const res = await fetch(
	// 		`https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${symbol}&outputsize=${outputsize}&apikey=${localStorage.getItem(
	// 			"keyST"
	// 		)}`
	// 	).then(response => response.json());
	// 	return res;
	...
	// static async TIME_SERIES_INTRADAY(
	// 	symbol,
	// 	interval = "5min",
	// 	outputsize = "compact"
	// ) {
	// 	const res = await fetch(
	// 		`https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=${symbol}&interval=${interval}&outputsize=${outputsize}&apikey=${localStorage.getItem(
	// 			"keyST"
	// 		)}`
	// 	).then(response => response.json());
	// 	return res;
	...
	static async GLOBAL_QUOTE(symbol, apikey) {
